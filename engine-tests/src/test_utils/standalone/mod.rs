...
impl StandaloneRunner {
    ...
    pub fn view_call(&mut self, args: ViewCallArgs) -> TransactionStatus {
        let io = self
            .storage
            .access_engine_storage_at_position(self.env.block_height + 10, 0, &[]);
        ...
        let engine = engine::Engine::new(args.sender, current_account_id, io, &env).unwrap();
